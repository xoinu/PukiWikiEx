...
// $_Id: edit.inc.php,v 1.40 2006/03/21 14:26:25 henoheno Exp $
// Copyright (C) 
...
//
// ParaeditId: $Id: edit.inc.php,v 1.1 2012/03/19 18:21:25 yamazaki Exp $
//
...
function plugin_edit_action()
{
	...
	if (! empty($vars['id'])) { //paraedit.
		$postdata = plugin_edit_parts($vars['id'], $source);
		if ($postdata === FALSE) {
			unset($vars['id']);
			$postdata = $vars['original']; // no sign for paraedit not found. Use standard procedure.
		}
	}
...
function plugin_edit_inline()
{
    ...
    if ($_paraedit && $isfreeze) return ''; //paraedit: Show nothing
	...
	if ($fixed_heading_anchor_edit && $editable && $ispage && ! $isfreeze) {
		...
		if (defined('PKWK_PARAEDIT_ICON')) {
			$icon = PKWK_PARAEDIT_ICON;
		}
		else {
...
function plugin_edit_write()
{
	...
	if ($digest != $oldpagemd5) {
		...
		list($postdata_input, $auto) = do_update_diff($oldpagesrc, $msg, $original);
          ...
          //paraedit: in case confilict happen, fallback to standad full page editting.
        unset($vars['id']); 
	...
	if ($add) {
        ...
        } else { // prevernt $msg destroy original messages.
...
// for paraedit:
// Extrace specified soource section for edit/preview and 
// replace specified soource section for write.
