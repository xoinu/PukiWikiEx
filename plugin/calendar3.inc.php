<?php
// $Id: calendar3.inc.php,v 1.2 2012/03/20 17:34:32 yamazaki Exp $
//
// Calendar3 plugin
//
// Usage:
//	#calendar3({[pagename|*],[yyyymm],[off]})
//	off: Don't view today's

function plugin_calendar3_convert()
{
	global $script, $vars, $post, $get, $weeklabels, $WikiName, $BracketName;
	global $_calendar3_plugin_edit, $_calendar3_plugin_empty;
   ...
   <table class="style_calendar" cellspacing="1" border="0" summary="calendar body">
    <tr>
     <td class="style_td_caltop" colspan="7">
      <a href="$script?plugin=calendar3&amp;file=$r_base&amp;date=$prev_date_str">&lt;&lt;</a>
      <strong>$m_name</strong>
      <a href="$script?plugin=calendar3&amp;file=$r_base&amp;date=$next_date_str">&gt;&gt;</a>
EOD;
	...
	if ($today_view) {
		...
		if (is_page($tpage)) {
			...
			$str .= '<hr /><a href="' . $script .
				'?cmd=edit&amp;page=' . $r_tpage . '">' .
				$_calendar3_plugin_edit . '</a>';
			$get['page'] = $post['page'] = $vars['page'] = $_page;
		} else {
			$str = sprintf($_calendar3_plugin_empty,
...
function plugin_calendar3_action()
{
	global $vars;
	...
	$ret['body'] = call_user_func_array('plugin_calendar3_convert', $aryargs);
